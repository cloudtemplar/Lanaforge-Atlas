...
- Regions can be highlighted **orange**; hovering one shows its name; zooming in reveals
  the people's names.
...
- Sub-region = ISO 3166-2: `BR-SP`, `US-CA`, `CA-ON`, …
  Look up exact codes in `iso-reference.md`.
...
arc-length resampling) runs at build time, the client just draws precomputed points.
